...
- (void)setDataSize:(NSInteger)newNumSamples {
    ...
    for (MLDataSet *values in self.totalVRAMDataSets) {
        [values resize:newNumSamples];
    }
    for (MLDataSet *values in self.freeVRAMDataSets) {
        [values resize:newNumSamples];
    }
    for (MLDataSet *values in self.cpuWaitDataSets) {
        [values resize:newNumSamples];
    }
    for (MLDataSet *values in self.utilizationDataSets) {
        [values resize:newNumSamples];
    }
...
- (void)setNumberOfGPUs:(NSInteger)newNumGPUs {
    ...
    // Add new MLDataSets if needed.
    for (NSInteger i = 0; i < newNumGPUs; i++) {
        if (newTotal.count <= i) {
            MLDataSet *s = [[MLDataSet alloc] init];
    ...
    // Remove extra MLDataSets if needed.
...
- (void)getLatestGraphicsInfo {
    ...
    NSMutableArray *graphicsCards = [NSMutableArray array];        // An array of MLGraphicsCard objects.
    ...
    for (NSInteger i = 0; i < numValues; i++) {
        ...
        if ([MLGraphicsCard matchingPCIDevice:pciD accelerator:accelD]) {
            // Matched.  Let's go with it.
            MLGraphicsCard *card = [[MLGraphicsCard alloc] initWithPCIDevice:pciD accelerator:accelD];
            ...
            for (NSInteger j = 0; j < accelerators.count; j++) {
                ...
                if ([MLGraphicsCard matchingPCIDevice:pciD accelerator:accelerators[j]]) {
                    // Found a match.
                    MLGraphicsCard *card = [[MLGraphicsCard alloc] initWithPCIDevice:pciD accelerator:accelerators[j]];
                    if (card) [graphicsCards addObject:card];
                    [pciIndicesUsed addIndex:i];
    ...
    if (pciDevices.count > 0) {
        for (NSInteger i = 0; i < pciDevices.count; i++) {
            ...
            for (NSInteger j = 0; j < accelerators.count; j++) {
                ...
                // Match these devices.
                MLGraphicsCard *card = [[MLGraphicsCard alloc] initWithPCIDevice:pciDevices[i] accelerator:accelerators[j]];
                if (card) [graphicsCards addObject:card];
                [pciIndicesUsed addIndex:i];
                [accelIndicesUsed addIndex:j];
            }
        ...
        for (NSInteger i = 0; i < accelerators.count; i++) {
            ...
            MLGraphicsCard *card = [[MLGraphicsCard alloc] initWithPCIDevice:nil accelerator:accelerators[i]];
            if (card) [graphicsCards addObject:card];
            [accelIndicesUsed addIndex:i];
        }
    }
...
- (NSString *)vendorString {
    if (self.vendor == MLPCIVendorAMD) {
        return @"AMD";
    }
    else if (self.vendor == MLPCIVendorNVidia) {
        return @"nVidia";
    }
    else if (self.vendor == MLPCIVendorIntel) {
        return @"Intel";
    }
    else if (self.vendor == MLPCIVendorApple) {
        return @"Apple";
    }
    else {
        return nil;
